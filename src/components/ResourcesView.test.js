...
describe('DocumentViewer', () => {
  ...
  it('renders detailed error information with attempted paths', async () => {
    ...
    try {
        ...
        root.render(
          <DocumentViewer
            ...
            error={{
              ...
              attemptedPaths: [
                { label: 'Netlify Blob', path: '/.netlify/functions/user-blob-access?key=path%2Fto%2Fdocument.pdf' },
      ...
      expect(primaryPathContainer.textContent).toContain('/.netlify/functions/user-blob-access?key=path%2Fto%2Fdocument.pdf');

      const pathEntries = Array.from(
        container.querySelectorAll('[data-testid="document-viewer-error-path"]')
      ).map((node) => node.textContent);
      expect(pathEntries).toContain('/.netlify/functions/user-blob-access?key=path%2Fto%2Fdocument.pdf');
...
describe('ResourcesView component', () => {
  it('fetches Netlify blob documents directly from metadata when available', async () => {
    ...
    try {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        '/.netlify/functions/user-blob-access?key=rag-documents%2Fuser%2Fdoc-123.pdf',
        { credentials: 'include' }
      );
  ...
  it('falls back to base64 content when Netlify downloads fail with 404', async () => {
    ...
    try {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        '/.netlify/functions/user-blob-access?key=rag-documents%2Fuser%2Fdoc-999.pdf',
        { credentials: 'include' }
      );
  ...
  it('surfaces a storage not found message when Netlify blob 404s without fallback content', async () => {
    ...
    try {
      ...
      expect(primaryPath.textContent).toContain('/.netlify/functions/user-blob-access?key=rag-documents%2Fuser%2Fmissing.pdf');
  ...
  it('shows attempted document paths when downloads fail', async () => {
    ...
    try {
      ...
      const primaryPathContainer = container.querySelector('[data-testid="document-viewer-error-primary-path"]');
      expect(primaryPathContainer).not.toBeNull();
      expect(primaryPathContainer.textContent).toContain('/.netlify/functions/user-blob-access?key=rag-documents%2Fuser%2Fdoc-404.pdf');
      ...
      expect(pathEntries).toContain('/.netlify/functions/user-blob-access?key=rag-documents%2Fuser%2Fdoc-404.pdf');
...
describe('buildNetlifyBlobDownloadUrl', () => {
  ...
  it('constructs a blob url from path metadata', () => {
    ...
    expect(url).toBe('/.netlify/functions/user-blob-access?key=rag-documents%2Fuser%2Ffile.pdf');
  });
  ...
  it('constructs a blob url from store and key metadata', () => {
    ...
    expect(url).toBe('/.netlify/functions/user-blob-access?key=rag-documents%2Frag-documents%2Fuser%2Ffile.pdf');
  });
  ...
  it('avoids double encoding already escaped path segments', () => {
    ...
    expect(url).toBe('/.netlify/functions/user-blob-access?key=rag-documents%2Fuser%2FMy%2520File%2520(1).pdf');
  });
  ...
  it('preserves encoded forward slashes within segments', () => {
    ...
    expect(url).toBe('/.netlify/functions/user-blob-access?key=rag-documents%2Fuser%2Fsome%252Fnested%252Fname.txt');
  });
