...
function createWordDocumentContent(studyData, resources) {
  ...
  const resourceSection = safeResources.length
    ? safeResources
        .map((resource, index) => {
          const title = resource?.title || `Resource ${index + 1}`;
          const type = resource?.type || 'Reference';
          const url = resource?.url || 'Not provided';
  ...
  const content = [
    ...
    bodyContent,
    '',
    `ADDITIONAL LEARNING RESOURCES (${resourceCount} items):`,
    '',
    resourceSection,
    '',
    '---',
...
function createFallbackStudyNotesData(studyNotesMessage, resources) {
  if (!studyNotesMessage) {
    return null;
  }

  const fallbackDate = new Date();
  const timestamp = studyNotesMessage.timestamp
    ? new Date(studyNotesMessage.timestamp)
    : fallbackDate;
  const hasValidTimestamp = !Number.isNaN(timestamp.getTime());
  const generatedDateValue = hasValidTimestamp ? timestamp : fallbackDate;

  const content =
    typeof studyNotesMessage.content === 'string'
      ? studyNotesMessage.content.trim()
      : '';
  ...
  const sourceMessages = Array.isArray(studyNotesMessage.sourceMessages)
    ? studyNotesMessage.sourceMessages
    : [];

  const selectedConversationIds = Array.from(
    new Set(
      sourceMessages
        .map((source) => source?.conversationCardId)
        .filter(Boolean)
    )
  );

  return {
    generatedAt: generatedDateValue.toISOString(),
